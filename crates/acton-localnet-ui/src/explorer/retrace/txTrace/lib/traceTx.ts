...
async function doTraceTree(
  ...
  try {
    ...
    throw mapRetraceError(error)
  }
}

function mapRetraceError(error: unknown): TxTraceError {
  ...
  if (/status code 429|HTTP 429|\(429\)/i.test(message)) {
    return new TooManyRequests(undefined, error)
  }

  if (/status code 422|HTTP 422|\(422\)/i.test(message)) {
    return new TxHashInvalidError(undefined, error)
  }

  if (/not found|cannot find trace|cannot find transaction/i.test(message)) {
    return new TxNotFoundError(undefined, error)
  }
  ...
  return new TxTraceError(message, error)
}
