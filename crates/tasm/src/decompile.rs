...
impl Disassembler {
    ...
    pub fn load_instruction(&self, slice: &mut CellSlice) -> Instruction {
        let list = &self.list;

        let bits = std::cmp::min(slice.size_bits(), MAX_OPCODE_BITS);
        let opcode = slice.get_uint(0, bits).unwrap() << (MAX_OPCODE_BITS - bits);
        ...
        while j - i > 1 {
            ...
            let k_element = &list[k];
            if k_element.min <= opcode as i64 {
                i = k;
            } else {
                j = k;
            }
        ...
        if layout.args.0.len() == 2
            && let Arg::InlineDictArg(_) = &layout.args.0[0]
        {
            let key_length = slice.load_uint(10).unwrap();
            let mut dict_slice = slice.load_reference().unwrap().as_slice().unwrap();
            ...
            let mut methods = vec![];
            for kv in dict.iter() {
                let Ok((key, mut value)) = kv else { continue };
                let mut key_slice = key.as_data_slice();

                let id = key_slice.load_uint(key_length as u16).unwrap();
                let code = self.decompile_slice(&mut value);
                methods.push(Method {
                    id,
            ...
            for child in &layout.args.0 {
                self.process_arg(child, slice, &mut args).unwrap();
            }
        }
    ...
    pub fn decompile_cell(&self, cell: &Cell) -> Code {
        let mut slice = cell.as_slice().unwrap();
        self.decompile_slice(&mut slice)
    }

    pub fn decompile_dyn_cell(&self, cell: &DynCell) -> Code {
        let mut slice = cell.as_slice().unwrap();
        self.decompile_slice(&mut slice)
    }

    pub fn decompile_slice(&self, slice: &mut CellSlice) -> Code {
        ...
        while slice.size_refs() > 0 {
            let ref_cell = slice.load_reference().unwrap();
...
mod tests {
    ...
    pub fn test_disassemble_jetton_minter() {
        ...
        let code = disassembler.decompile_cell(&code);
