...
fn failure(error: impl std::fmt::Display) -> EnvironmentRuntimeError {
...
impl FullTonNetworkDriver {
    ...
    pub(crate) async fn saved_admin_operation(
    ...
    ) -> Result<Option<AdminOperation>, EnvironmentRuntimeError> {
        let dir = self.compose_file.with_file_name("admin-operations");
        ...
        if op.is_active() {
            ...
            op.error = Some("Studio restarted before the operation result was recorded. Recovery ran before startup; inspect the account before submitting another edit.".into());
    ...
    pub(crate) async fn admin_is_running(&self) -> bool {
        ...
        let Ok(output) = self
            .command_output(
                command,
                "check environment after an administrative operation",
        ...
        states
            .iter()
            .any(|s| s.service == "localton" && s.state == "running")
            && states.iter().all(|s| {
                !s.failed()
                    && (s.state == "running"
                        || matches!(
                            s.service.as_str(),
                            "v3-basechain-bootstrap" | "v3-migrations"
                        ))
            })
    }
    ...
    async fn save_recovery(&self, journal: &Recovery) -> Result<(), EnvironmentRuntimeError> {
    ...
    pub(crate) async fn recover_admin(&self) -> Result<(), EnvironmentRuntimeError> {
        let path = self.compose_file.with_file_name(JOURNAL);
        if !path.exists() {
            return Ok(());
        }
    ...
    ) -> Result<u32, EnvironmentRuntimeError> {
    ...
    async fn admin_work(
        ...
        let url = format!(
            ...
            self.compose_config.api_v3_port
        );
        let deadline = Instant::now() + ADMIN_TIMEOUT;
        loop {
    ...
    async fn wait_ready(&self) -> Result<(), EnvironmentRuntimeError> {
        let deadline = Instant::now() + ADMIN_TIMEOUT;
        loop {
    ...
    async fn start_core(&self, services: &[String]) -> Result<(), EnvironmentRuntimeError> {
        let mut command = self.compose_command();
    ...
    async fn start_all(&self) -> Result<(), EnvironmentRuntimeError> {
        let mut command = self.compose_command();
    ...
    async fn wait_live(
        ...
        service: &str,
        action: &str,
    ) -> Result<serde_json::Value, EnvironmentRuntimeError> {
        let deadline = Instant::now() + ADMIN_TIMEOUT;
        loop {
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires Docker and ACTON_STUDIO_LOCALTON_IMAGE built with localton-admin-dev"]
    async fn administrative_hardfork_and_rollback_on_two_nodes() {
        assert!(std::env::var("ACTON_STUDIO_LOCALTON_IMAGE").is_ok());
        let dir = tempfile::tempdir_in("/tmp").unwrap();
        let nodes = vec![FullTonNode {
            id: "node-1".into(),
            name: "replica".into(),
            validator: false,
            port_base: 19000,
        ...
        let result: Result<(), EnvironmentRuntimeError> = async {
            ...
            request.validate()?;
            let operation = RwLock::new(Some(AdminOperation { id: request.id().into(), phase: "preparing".into(), started_at: chrono::Utc::now().to_rfc3339(), finished_at: None, error: None, block_seqno: None }));
            let mut running = Box::pin(driver.apply_admin(&nodes, &request, &operation));
            let mut previous = String::new();
            let seqno = loop {
                tokio::select! {
                    result = &mut running => break result?,
                    _ = sleep(Duration::from_secs(2)) => {
                        let current = operation.read().await.as_ref().unwrap().phase.clone();
                        if current != previous { eprintln!("Admin phase: {current}"); previous = current; }
                    }
                }
            };
            ...
            driver.apply_admin(&nodes, &changed, &operation).await?;
            ...
            let error = driver.apply_admin(&nodes, &invalid, &operation).await.err().ok_or_else(|| failure("Invalid edit was accepted"))?;
