 ...
 * @returns {number}
 */
export function find_nonce(challenge, difficulty, start_nonce, max_attempts) {
    const ptr0 = passStringToWasm0(challenge, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.find_nonce(ptr0, len0, difficulty, start_nonce, max_attempts);
    if (ret[2]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('acton_faucet_pow_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
