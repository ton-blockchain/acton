 ...
 */
export function find_nonce(challenge: string, difficulty: number, start_nonce: number, max_attempts: number): number;

export type InitInput = RequestInfo | URL | Response | BufferSource | WebAssembly.Module;

export interface InitOutput {
  readonly memory: WebAssembly.Memory;
  readonly find_nonce: (a: number, b: number, c: number, d: number, e: number) => [number, number, number];
