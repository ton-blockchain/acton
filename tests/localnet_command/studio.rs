...
async fn studio_reports_docker_recovery_and_restarts_the_same_environment_after_fixing_it() {
    ...
    expect![[r#"
        ...
        Next step: Start Docker Desktop or your Docker Engine service and wait until `docker info` succeeds, then retry this operation"#]].assert_eq(&error.lines().take(2).collect::<Vec<_>>().join("\n"));
