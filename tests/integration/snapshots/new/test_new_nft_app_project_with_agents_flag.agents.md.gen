...
- Treat the contracts under `contracts/src/` as the source of truth, especially `NftCollection.tolk`, `NftItem.tolk`, `messages.tolk`, `types.tolk`, and `errors.tolk`.
- Treat the collection and item contracts as a coupled system. Keep storage, message formats, tests, wrappers, TypeScript wrappers, scripts, and frontend flows consistent across both sides.
- Treat `wrappers-ts/NftCollection.gen.ts` and `wrappers-ts/NftItem.gen.ts` as generated output. Prefer regenerating them from the contract ABI instead of hand-editing them when the ABI changes.
