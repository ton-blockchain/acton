...
use crate::commands::test::{INTERNAL_DISABLE_STUDIO_REPORTER_ENV, TestDescriptor};
use crate::formatter::FormatterContext;

const STUDIO_EVENT_QUEUE_CAPACITY: usize = 256;
...
impl TestReporter for StudioReporter {
    ...
    fn on_test_finished(&mut self, test: &TestReport) -> anyhow::Result<()> {
        ...
        let live_report = self.event_worker.is_some().then(|| report.clone());
        if let Some(existing) = self.run.reports.iter_mut().find(|candidate| {
...
fn start_event_worker(
    ...
    let client = reqwest::blocking::Client::builder()
        ...
        .ok()?;
    let endpoint = format!(
        "{studio_url}{STUDIO_TEST_RUNS_PATH}/{}/events",
