...
- Treat the contracts under `contracts/` as the source of truth, especially `jetton-minter-contract.tolk`, `jetton-wallet-contract.tolk`, `messages.tolk`, `storage.tolk`, and `errors.tolk`.
...
- Keep `tests/wallet.test.tolk`, `tests/wrappers/JettonMinter.tolk`, `tests/wrappers/JettonWallet.tolk`, and `scripts/deploy.tolk` aligned with contract changes.
- When ABI changes are involved, prefer regenerating wrappers with `acton wrapper jetton_minter` and `acton wrapper jetton_wallet` over hand-editing wrapper files.
