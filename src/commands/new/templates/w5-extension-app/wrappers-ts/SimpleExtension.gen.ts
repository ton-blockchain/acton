...
export const CollectPaymentFromWallet = {
    ...
    toCell(self: CollectPaymentFromWallet): c.Cell {
        return makeCellFrom<CollectPaymentFromWallet>(self, CollectPaymentFromWallet.store);
    }
}
...
export const WithdrawExtensionBalance = {
    ...
    toCell(self: WithdrawExtensionBalance): c.Cell {
        return makeCellFrom<WithdrawExtensionBalance>(self, WithdrawExtensionBalance.store);
    }
}
...
export class SimpleExtension implements c.Contract {
    ...
    static createCellOfCancelSubscription(body: {
        queryId: uint64
    }) {
    ...
    async sendCancelSubscription(provider: ContractProvider, via: Sender, msgValue: coins, body: {
        queryId: uint64
    }, extraOptions?: ExtraSendOptions) {
        return provider.internal(via, {
            value: msgValue,
    ...
    async sendReceivePaymentFromWallet(provider: ContractProvider, via: Sender, msgValue: coins, body: {
    }, extraOptions?: ExtraSendOptions) {
        return provider.internal(via, {
            value: msgValue,
    ...
    async getExtensionInfo(provider: ContractProvider): Promise<ExtensionStorage> {
        const r = StackReader.fromGetMethod(5, await provider.get('extensionInfo', []));
        return ({
            $: 'ExtensionStorage',
            walletAddress: r.readSlice().loadAddress(),
            admin: r.readSlice().loadAddress(),
            subscriptionAmount: r.readBigInt(),
            lastPaymentTime: r.readNullable<uint32>(
                (r) => r.readBigInt()
            ),
