...
export const PayloadInline = {
  ...
  create(args: { value: RemainingBitsAndRefs }): PayloadInline {
    return {
      $: 'PayloadInline',
      ...args,
    };
...
export const PayloadInRef = {
  ...
  create(args: { value: CellRef<RemainingBitsAndRefs> }): PayloadInRef {
    return {
      $: 'PayloadInRef',
      ...args,
    };
 ...
 */
export type Payload = PayloadInline | PayloadInRef;

export const Payload = {
  fromSlice(s: c.Slice): Payload {
    return s.loadBoolean()
      ? PayloadInRef.fromSlice(s)
      : PayloadInline.fromSlice(s);
  },
...
export const AskToChangeOwnership = {
  ...
  store(self: AskToChangeOwnership, b: c.Builder): void {
    ...
    storeTolkNullable<c.Cell>(self.customPayload, b, (v, b) => b.storeRef(v));
...
export const RequestStaticData = {
  ...
  create(args: { queryId: uint64 }): RequestStaticData {
    return {
      $: 'RequestStaticData',
      ...args,
    };
...
export const NotificationForNewOwner = {
  ...
  toCell(self: NotificationForNewOwner): c.Cell {
    return makeCellFrom<NotificationForNewOwner>(
      self,
      NotificationForNewOwner.store,
    );
  },
};
...
export const ReturnExcessesBack = {
  ...
  create(args: { queryId: uint64 }): ReturnExcessesBack {
    return {
      $: 'ReturnExcessesBack',
      ...args,
    };
...
export const NftDataReply = {
  ...
  fromSlice(s: c.Slice): NftDataReply {
    throw new Error(
      `Can't unpack 'NftDataReply' from cell, because 'NftDataReply.itemIndex' is 'int' (not int32/uint64/etc.)`,
    );
  },
  store(self: NftDataReply, b: c.Builder): void {
    throw new Error(
      `Can't pack 'NftDataReply' to cell, because 'self.itemIndex' is 'int' (not int32/uint64/etc.)`,
    );
  },
...
export class NftItem implements c.Contract {
  ...
  static fromStorage(
    ...
    const initialState = {
      ...
      data: NftItemStorage.toCell(NftItemStorage.create(emptyStorage)),
    };
  ...
  static createCellOfRequestStaticData(body: { queryId: uint64 }) {
  ...
  async sendAskToChangeOwnership(
    provider: ContractProvider,
    via: Sender,
    msgValue: coins,
    body: {
  ...
  async getNftData(provider: ContractProvider): Promise<NftDataReply> {
    ...
    return {
      ...
      ownerAddress: r.readNullable<c.Address>((r) =>
        r.readSlice().loadAddress(),
      ),
      content: r.readNullable<string>((r) => r.readSnakeString()),
    };
  }
}
