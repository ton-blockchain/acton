...
- Treat the contracts under `contracts/` as the source of truth, especially `NftCollection.tolk`, `NftItem.tolk`, `messages.tolk`, `types.tolk`, `fees-management.tolk`, and `errors.tolk`.
...
- Keep `tests/nft-collection.test.tolk`, `tests/nft-item.test.tolk`, generated wrappers, `wrappers/utils.tolk`, and `scripts/` aligned with contract changes. Treat `wrappers/utils.tolk` as hand-written helper code.
