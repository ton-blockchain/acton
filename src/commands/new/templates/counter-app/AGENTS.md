...
- Treat `wrapper-ts/Counter.ts` as generated output. Prefer regenerating it from the contract ABI instead of hand-editing it when the ABI changes.
- Keep `contracts/tests/`, `contracts/wrappers/Counter.tolk`, `contracts/scripts/deploy.tolk`, `wrapper-ts/Counter.ts`, and the frontend code in `app/` aligned with contract changes.
