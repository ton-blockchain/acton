...
- Treat `wrappers-ts/Counter.gen.ts` as generated output. Prefer regenerating it from the contract ABI instead of hand-editing it when the ABI changes.
- Keep `contracts/tests/`, `contracts/wrappers/Counter.gen.tolk`, `contracts/scripts/deploy.tolk`, `wrappers-ts/Counter.gen.ts`, and the frontend code in `app/` aligned with contract changes.
