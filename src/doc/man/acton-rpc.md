...
- compiler ABI JSON, including files emitted under `build/abi`
- `.tolk` source or interface files, including `*.types.tolk`; these are
...
1. use `--abi` directly when it is provided to `rpc info` or `rpc call`
2. otherwise fetch the remote account code cell
3. compute its `code_hash`
4. compare that hash with locally configured contracts
5. fall back to the bundled ABI catalog when there is no local ABI
6. query the verifier API when no local or bundled ABI matches
7. cache successful verifier responses for 24 hours and use stale entries
   during verifier outages
8. decode storage and get-method results with the selected compiler ABI
...
4. Decode account storage with an explicit Tolk interface:

   ```bash
   acton rpc info EQC... --abi contracts/Counter.types.tolk --net mainnet
   ```

5. Call a get-method with an explicit compiler ABI JSON file:

   ```bash
   acton rpc call EQC... get_wallet_data --abi build/abi/counter.json --net mainnet
   ```

6. Call a get-method with automatically matched ABI metadata:

   ```bash
   acton rpc call EQC... get_wallet_data --net mainnet
   ```

7. Call a get-method by numeric TVM method id:

   ```bash
   acton rpc call EQC... 85143 --net mainnet
   ```

8. Call a get-method without ABI metadata by passing raw stack arguments:

   ```bash
   acton rpc call EQC... 85143 0xff true cell:B5EE9C... --net mainnet
   ```

9. Pass `addr_none` to an `any_address` get-method argument:

   ```bash
   acton rpc call EQC... accepts_any_address addr_none --net localnet
   ```

10. Use a custom network defined in another manifest:

   ```bash
   acton --manifest-path ../incident/Acton.toml rpc info EQC... --net custom:staging
   ```

11. Print the latest mainnet masterchain block JSON:

   ```bash
   acton rpc block --net mainnet
   ```

12. Print the latest mainnet masterchain block number:

   ```bash
   acton rpc block-number --net mainnet
   ```

13. Print a transaction trace from localnet:
