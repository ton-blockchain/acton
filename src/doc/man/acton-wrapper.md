...
## COMMON GENERATED HELPER SHAPES

Generated Tolk wrappers usually expose the same family of helpers:

- `fromStorage(...)` to construct a wrapper together with `StateInit` and the
  computed deployment address
- sometimes `fromAddress(...)` when the template or generator needs a handle to
  an already deployed contract without local init data
- `deploy(...)` to send the deploy message using the wrapper's stored
  `StateInit`
- `send{Name}(...)` helpers for each incoming message declared in the ABI
- `sendAny(...)` for a raw custom body cell when you need to bypass typed
  message helpers
- typed get-method helpers emitted from declared getter methods

The exact method set depends on the ABI. Template wrappers copied by
`acton new` are starter files checked into the scaffold and may lag behind or
slightly differ from the latest generator output, but they follow the same
general helper patterns.
