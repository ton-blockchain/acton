...
registered in the Acton verifier.

#### Synopsis

`acton doc abi` _contract-or-code-hash_

#### Options

{{#options command="acton doc abi"}}

{{#option "_contract-or-code-hash_" }}
Local or bundled contract name, local contract id, or verifier code hash.
{{/option}}
...
back to the bundled ABI catalog. If the argument is a 64-character hexadecimal
code hash, optionally prefixed with `0x`, Acton queries the verifier.
...
- verifier lookup uses the same 24-hour project cache as `acton rpc`
- an expired verifier cache entry remains available during verifier outages
