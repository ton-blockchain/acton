...
<Callout type="info" title="Background">
  Usage of on-chain libraries is covered in the corresponding [TON documentation guide](https://docs.ton.org/contracts/techniques/using-on-chain-libraries).
</Callout>
...
Before publishing, Acton checks local and global metadata for the same code hash
and network. In interactive mode, it can offer to top up a tracked library instead.
...
- **Remaining**: estimated storage runway from Acton's built-in masterchain storage prices and time since the last top-up.
