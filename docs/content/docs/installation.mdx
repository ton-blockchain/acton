...
<Callout type="info" title="Support policy">
    Acton is stable on the latest major numbered release. First-class support covers versioned releases on macOS (ARM64 and x86_64) and Linux GNU (x86_64 and ARM64). For Linux, the documented baseline is Ubuntu 22.04 or newer.

    `trunk` builds, WSL installs, and other source-built targets are beta or best-effort surfaces. Native Windows is not supported: install Acton inside WSL with Ubuntu 22.04 or newer and follow the Linux steps.
...
<Tabs items={["macOS", "Linux", "WSL"]}>
    ...
    <Tab value="WSL">
        ...
        Run this inside the WSL shell with Ubuntu 22.04 or newer, not in PowerShell nor in `cmd.exe`:
...
<Callout type="warning" title="Use WSL on Windows">
    On Windows, use WSL with Ubuntu 22.04 or newer; native Windows is not supported.

    Run Git and Acton inside the same WSL distribution. In VS Code, install the
    [WSL extension](https://code.visualstudio.com/docs/remote/wsl), then use
    **WSL: Reopen Folder in WSL**. You can also run `code .` from the project directory
    inside WSL. This also applies to project folders on Windows drives, such as `/mnt/c`.

    A WSL terminal alone does not change the environment for Git commands from the
    Source Control view. If a Git hook reports `acton was not found in PATH`,
    make sure that VS Code shows the correct WSL distribution in its status bar.
</Callout>
...
```bash
docker run --rm ghcr.io/ton-blockchain/acton:{{ acton_version }} --version
```
