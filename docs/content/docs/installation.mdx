...
<Callout type="info" title="Support policy">
    First-class support covers versioned releases on macOS (ARM64 and x86_64) and Linux GNU (x86_64 and ARM64). For Linux, the documented baseline is Ubuntu 22 or newer. The `trunk` builds and WSL installs are beta or best-effort surfaces. Native Windows is not supported: install Acton inside WSL with Ubuntu 22 or newer and follow the Linux steps.
