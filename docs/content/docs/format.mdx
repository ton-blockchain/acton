...
Use `--stdin` for editor integrations. `--stdin-filepath` sets the path shown in diagnostics:

```bash
acton fmt --stdin --stdin-filepath contracts/main.tolk < contracts/main.tolk
```

Acton writes the formatted source to standard output. It does not read or write
the virtual path. Positional file and directory arguments cannot be used with `--stdin`.
