...
```bash
acton fmt --manifest-path ../my-project/Acton.toml
```

Or set the project root directly:

```bash
acton fmt --project-root ../my-project
```

<Callout type="warn">
  Use either `--manifest-path` or `--project-root`, not both.
</Callout>
...
### `acton fmt` reports syntax errors
