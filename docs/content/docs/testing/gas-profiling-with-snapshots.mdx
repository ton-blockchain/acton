...
## Measure a callback or one transaction

<AvailabilityBadge since="Acton 1.2" />

Use [`bench.measure()`](/docs/standard_library/testing/bench#benchmeasure) to
measure a callback independently of a transaction chain:

```tolk
import "@acton/testing/bench"

val profile = bench.measure("encode payload", fun() {
    return beginCell().storeUint(42, 32).endCell();
});
println(bench.format(profile));
expect(profile.result.beginParse().loadUint(32)).toEqual(42);
```

The result includes the callback value, gas usage, instruction statistics, and
TASM. `bench.formatDiff(baseline, current)` compares two measurements.

For one transaction in a trace, use
[`SendResultList.getUsedGas()`](/docs/standard_library/emulation/network#sendresultlistgetusedgas):

```tolk
val gas = txs.getUsedGas<IncreaseCounter>({ to: counter.address });
expect(gas).toBeLess(10000);
```

The helper returns gas from the first matching transaction. If none matches,
it fails with the search parameters and transaction tree.
