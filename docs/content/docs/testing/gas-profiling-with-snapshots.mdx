...
Gas profiling records gas and fee metrics for message opcodes and transaction chains. Performance regressions show up in the `--snapshot` and `--baseline-snapshot` outputs. For source-level hot paths, Acton can also write gas-weighted execution profiles with `--gas-profile`.
...
gas deltas. For flamegraph-style tooling, use folded stacks:
