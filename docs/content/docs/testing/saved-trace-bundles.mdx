...
- Regular test runs do not create trace artifacts unless trace export, Test UI, or Studio reporting enables them.
  Since Acton 1.2, an active Studio reporter collects traces for the saved run.
...
- `skipped_traces_count`: optional count of trace chains hidden from the saved bundle with `hideTraceFromUi()`, added in Acton 1.2. The field is omitted when the count is zero.
...
- `is_treasury_deploy`: optional marker for treasury deployment traces, added in Acton 1.2 and omitted when false.
...
Hidden traces are omitted from `traces[]`, while the exported root file records how many chains were skipped in `skipped_traces_count`. The transactions still run normally and remain available to the test code for assertions after they are hidden.
