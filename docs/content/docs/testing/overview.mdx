...
<Callout type="tip">
  [Acton Studio](/docs/studio) includes results and the transaction inspector in <kbd>[Local tests](/docs/studio#local-tests)</kbd>, alongside project-wide test run history.

  For now, coverage and gas profiling are only available in the standalone [Test UI](/docs/testing/test-ui/overview).
</Callout>
