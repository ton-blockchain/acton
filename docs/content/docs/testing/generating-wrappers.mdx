...
description: "Learn how to automatically generate wrappers for contracts"
---

Integration tests often need utility wrapper files to interact with smart contracts: send messages and call get-methods. The [`acton wrapper`](/docs/commands/wrapper) command generates those wrappers from contract ABI.

For precompiled `.boc` contracts, add a `types` interface file first. See [Precompiled BoC contracts](/docs/building/precompiled-boc).
...
Wrapper type names and default filenames use the PascalCase contract ID from
`Acton.toml`, independent of the source filename and `display-name`.
Regenerate wrappers after upgrading and update imports if those names differ.
...
get fun `test example`() {
    ...
    // 2. Deploy it
    val deployer = testing.treasury("deployer");
    contract.deploy(deployer.address, { value: grams("1") });
}
```

<AvailabilityBadge since="Acton 1.2" />

`fromStorage()` accepts an optional workchain after `toShard`. For example,
`Counter.fromStorage({ counter: 0 }, null, MASTERCHAIN)` creates a masterchain
address. The default is `BASECHAIN`, and `deploy()` preserves the selected workchain.
...
output-dir = "tests/counter-wrappers"
generate-test = false
...
- `acton wrapper Counter` writes `tests/counter-wrappers/Counter.gen.tolk` without a test stub because the contract overrides those two Tolk fields
- `acton wrapper Counter --ts` writes `app/src/wrappers/counter/Counter.gen.ts`
- contracts without nested wrapper settings continue to use `tests/generated-wrappers`, `tests/generated-tests`, and `wrappers-ts`

<AvailabilityBadge since="Acton 1.2" />

Per-contract settings use the same fields as the project defaults and inherit
any fields they omit. CLI path and test flags override both levels.
