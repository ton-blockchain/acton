...
This page is a collection of practical testing patterns. For the full API surface, use the [built-in matchers and helpers](/docs/testing/built-in-matchers-and-helpers) reference.

For complete real-world suites, use the [`acton-contracts` repository](https://github.com/ton-blockchain/acton-contracts) as a reference: it contains larger Jetton, NFT, wallet, multisig, DNS, elector, and config contract tests built with these patterns.

## Selectively run tests files and folders

Pass multiple files or directories to one test run. Overlapping selections run once:

```bash
acton test tests/unit tests/integration/wallet.test.tolk
```

<AvailabilityBadge since="Acton 1.2" />

Pass `--no-capture` for live test output. Start `acton studio` to retain test history and inspect results in the [Studio workspace](/docs/studio#local-tests).
...
See [gas profiling and snapshots](/docs/testing/gas-profiling-with-snapshots) for baseline reports and CI setup.
...
Remember that action indexing is reversed: index `0` is the last produced action. See [out-action helpers](/docs/testing/built-in-matchers-and-helpers#out-action-helpers) for details.

## Control time

<AvailabilityBadge since="Acton 1.2" />

Tests start with the current system Unix time. An explicit `--fork-block-number` uses that block's timestamp instead; a latest fork uses the current system time.
...
See [world state snapshots](/docs/testing/world-state-snapshots) for file layout and caveats.
...
See [step-by-step execution](/docs/testing/step-by-step-execution) for `executeTill(...)`, batching, and trace export behavior.
...
See [test attributes](/docs/testing/test-attributes) and [fuzz testing](/docs/testing/fuzz-testing) for the complete syntax.
...
The printed tree is the same format used by matcher failures. See [reading transaction chains](/docs/testing/reading-transaction-chains) for the format and [Test UI](/docs/testing/test-ui/overview) for interactive trace inspection.
