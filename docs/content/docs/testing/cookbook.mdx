...
Tests start with the current system Unix time. An explicit `--fork-block-number`
uses that block's timestamp instead; a latest fork uses the current system time.
...
`BlockchainConfigMap.setParamRaw()` and `getParamRaw()` accept signed 32-bit
parameter IDs, including negative IDs for out-of-consensus parameters.
