...
Test runs report to [Studio](/docs/studio/tests) when it is running for the same
...
`--no-fork-cache` disables persistent account and library cache reads and writes.
It does not disable the masterchain snapshot cache. See [fork caching](/docs/testing/fork-testing#cache-scope-and-refresh-behavior).

The fork uses state and network configuration from the same masterchain block.
When `fork-block-number` is set, it also uses that block's Unix time; otherwise,
