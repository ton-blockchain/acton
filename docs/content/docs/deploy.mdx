...
There is no dedicated `acton deploy` command. Instead, contracts are deployed using [Tolk scripts](/docs/commands/script). The same deployment script can be used for isolated emulation, persistent [virtual environments](/docs/environments/overview), and testnet or mainnet broadcasts.
...
- A [configured deployer wallet](/docs/wallets) with at least 1 GRAM for testnet and mainnet deployments. Alternatively, pass `--tonconnect` to use a wallet from an external wallet service through the TON Connect protocol.
...
The same pattern works for more complex contracts. In a project created with `acton new --template jetton`, use its metadata helper before calling the wrapper's deployment helper.
...
fun main() {
    ...
    val metadata = buildJettonMetadataCell({
        name: "ActonJetton",
        symbol: "ACTON",
    ...
    val trace = minter.deploy(deployer.address, { value: grams("0.05") }).waitForTrace();
    if (trace == null) {
      Assert.fail("Could not find the trace, deployment has failed!");
    }
