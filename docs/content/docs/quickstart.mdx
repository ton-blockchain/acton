...
<Steps>
    ...
    <Step title="Deploying contract">
        ...
        The generated scaffold also exposes the same deploy script through
        `acton run` aliases, so the equivalent shortcut is:

        ```bash
        acton run deploy-testnet
        ```

        If you want to exercise the same script against the local emulator
        first, use:
