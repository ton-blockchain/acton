...
<Callout type="idea">
  Pass `--hooks` to install pre-push checks, `--hooks=pre-commit` for checks before each commit, or `--agents` to add coding-agent guidance. See [`acton new`](/docs/commands/new) for all options. When run without arguments, `acton new` prompts interactively.
</Callout>
