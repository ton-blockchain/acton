...
The generated `Acton.toml` includes ready-to-run script aliases `deploy-emulation` and `deploy-testnet`. If [acton wallet](/docs/commands/wallet) is already configured, the contract can be deployed to testnet immediately.
...
See the [dApp integration guide](/docs/ui-setup) for more information about using TypeScript wrappers.
...
If the `--net` flag is passed, all messages sent by `net.send()` and `net.sendExternal()` will be applied to the real network.
