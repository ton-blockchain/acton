...
<Callout type="idea">
  Pass `--hooks` to install pre-push checks, `--hooks=pre-commit` for checks before each commit, or
  `--agents` to add coding-agent guidance. See [`acton new`](/docs/commands/new) for all options.
  When run without arguments, `acton new` prompts interactively.
</Callout>
...
Test UI's result and transaction inspector are also included in Acton Studio's <kbd>[Local tests](/docs/studio#local-tests)</kbd>, alongside project-wide test run history.

<Callout type="note">
  For now, coverage and gas profiling are only available in the standalone Test UI.
</Callout>
...
If automatic ABI matching is unavailable, pass compiler ABI JSON or a Tolk
interface explicitly:

```bash
acton rpc info <CONTRACT_ADDRESS> --abi build/abi/counter.json --net testnet
acton rpc call <CONTRACT_ADDRESS> currentCounter --abi contracts/Counter.types.tolk --net testnet
```
