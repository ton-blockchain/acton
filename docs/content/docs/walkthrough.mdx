...
This guide covers the full Acton workflow: creating a counter contract from template, compiling it, linting and formatting code, running tests with coverage and mutation analysis, setting up a wallet, deploying to testnet, and verifying the source.
...
- `--app` flag adds a Vite-based Reacet frontend scaffold and generated TypeScript wrappers alongside the Tolk sources.
...
Optional extras: pass `--hooks` to install pre-commit checks, or `--agents` to add coding-agent guidance. See [`acton new`](/docs/commands/new) for all options.

This command can also be run without arguments. It will prompt interactively.
```acton-cli
acton new
```
...
The generated `Acton.toml` includes ready-to-run script aliases `deploy-emulation` and `deploy-testnet`. If [acton wallet](/docs/commands/wallet) is already configured, then contract can be instantly deployed to testnet.

```acton-cli
acton run deploy-emulation --net testnet
```
...
Useful build extras: pass `--info` to print resolved project configuration, or `--clear-cache` to discard cached artifacts and recompile from scratch. See [`acton build`](/docs/commands/build) for all options.
...
Use [`acton check`](/docs/commands/check) to run the Tolk linter across the whole project and report diagnostics. On a clean project, it prints nothing and exits with status 0.
...
Wrappers are auto-generated from contract ABI, so every time it changes, wrappers should be regenerated. It is advised to store wrappers in VCS along with contract sources.
...
To quickly identify the place, where exception occured pass `--backtrace full` to get source locations for a failure without stepping through:
...
For cases, when simple error tracing is now enough, acton ships a source-level debugger that uses the [Debug Adapter Protocol (DAP)](https://microsoft.github.io/debug-adapter-protocol/). It runs the same contract code as production without a separate debug build.

It shows everything expected from a source-level debugger:
...
Use [`acton script`](/docs/commands/script) to deploy contracts in real network and interact with them. Additionally, Tolk scripts may be run in the same local emulation environment as tests.

Emulating scripts are useful for checking contract address derivation, initial state, deployment statuses, and post-deploy getter calls before deploying contracts to a public network.
...
This command emulated the script in the local environment. Additionally, scripts may be emulated against the real network state.
...
It is strictly recommended to verify scripts using `--fork-net` before executing them in the real network.

### Run a script against a real network

If `--net` flag is passed, then all messages sent by `net.send()` and `net.sendExternal()` will be applied to real network.
...
The counter template does not need on-chain libraries, but larger contracts often use them to keep reusable code in one masterchain account and reference it from many deployments. When a contract dependency in `Acton.toml` uses `kind = "library_ref"`, publish that dependency first with [`acton library`](/docs/commands/library), then build and deploy the contracts that reference it.
