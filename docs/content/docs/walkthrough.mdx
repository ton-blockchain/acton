...
```acton-cli
$ acton new first_counter --template counter --app
  Installing standard library v0.4.2
...
Mutants:  15
  ...
  ◉ Mutation 1/15 contracts/src/Counter.tolk:22:13 Remove assert statements KILLED
  ◉ Mutation 2/15 contracts/src/Counter.tolk:29:13 Remove assert statements SURVIVED
  ◉ Mutation 3/15 contracts/src/Counter.tolk:30:13 Remove assert statements KILLED
  ◉ Mutation 4/15 contracts/src/Counter.tolk:37:13 Remove assert statements SURVIVED
  ◉ Mutation 5/15 contracts/src/Counter.tolk:43:13 Remove assert statements KILLED
  ...
  ◉ Mutation 15/15 contracts/src/types.tolk:18:5 Remove contract.setData() method calls KILLED
...
────────────────────────────────────────────────────────────
  ...
  Level: critical
  Group: assertion
  at contracts/src/Counter.tolk:29:13
    27 │         DecreaseCounter => {
    28 │             var storage = lazy Storage.load();
    29 │             assert (storage.owner == in.senderAddress) throw Errors.NotOwner;

  ✗ Mutation #12
  Rule:  Replace >= with > [replace_greater_or_equal_with_greater_than]
  Level: major
  Group: comparison
  at contracts/src/Counter.tolk:30:37
```
...
The clean counter template passes. The output below shows the kind of source
locations Acton prints when a test fails:
...
Once the terminal shows `Debugger server listening on 127.0.0.1:4711`, open the **Run and Debug** panel, pick **Retrace DAP** from the configuration dropdown, and run it. VS Code connects to the listening port and the test launches under the debugger.
