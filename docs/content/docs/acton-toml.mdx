...
Acton bundles its Go generator; only a Go toolchain on PATH is required.
See [Go generation](/docs/commands/wrapper#go-generation) for toolchain requirements
and first-use dependency caching. There is no generator executable setting.
