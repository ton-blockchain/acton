...
[Acton Studio](/docs/studio) watches this manifest and rebuilds project artifacts.
...
Configures simulator defaults and the shared localnet target. The `acton simulator` command reads this table.
...
`db-path`, `response-delay-ms`, `block-time-ms`, `no-mining`, and `mine-empty-blocks` are available from Acton 1.2.
