...
The nested `wrappers.tolk`, `wrappers.typescript`, and `wrappers.go` tables accept the same
...
value. Per-contract Go overrides apply only to single-contract generation.
...
`acton build --out-dir <DIR>`, `acton build --gen-dir <DIR>`, `acton build --output-abi <DIR>`, `acton build --output-fift <DIR>`, and `acton build --output-sources <DIR>` override these values for a single run. Config values here are project-root-relative. The CLI flags stay relative to the current working directory unless absolute paths are passed. Precompiled `.boc` contracts emit ABI only when their contract config sets `types`; they never emit Fift or source registration output.
...
| `generate-test`   | `boolean` | `false` | Generate a Tolk test stub by default for Tolk wrapper runs. |
...
| Field        | Type     | Default          | Description |
|:-------------|:---------|:-----------------|:------------|
| `output-dir` | `string` | `wrappers-go`    | Directory for generated Go package files. |
| `package`    | `string` | `wrappers`       | Go package name. |
| `generator`  | `string` | `tolk-abi-to-go`  | Existing executable on PATH or a project-relative path such as `./bin/tolk-abi-to-go`. |

Acton does not install the generator. See [Go generation](/docs/commands/wrapper#go-generation)
for pinned installation and local-build guidance.
...
- Per-contract `[contracts.<name>.wrappers.tolk].generate-test` overrides `[wrappers.tolk].generate-test`. Both affect only Tolk wrapper generation. `--ts` and `--go` cannot be combined with test-stub generation.
- Go settings resolve from CLI overrides (`--output-dir`, `--go-package`, `--go-generator`), then `[contracts.<name>.wrappers.go]` for a single contract, then `[wrappers.go]`, then built-in defaults. `--all --go` generates one package and uses project-level settings, ignoring per-contract Go overrides. `--catalog FILE --go` also uses project-level settings when a manifest exists, but does not require a manifest or compile sources. `--output` is not supported for Go.
...
Configures shared defaults for the localnet target and local development services.

| Field               | Type       | Default | Description                                                                                                                 |
|:--------------------|:-----------|:--------|:----------------------------------------------------------------------------------------------------------------------------|
| `port`              | `integer`  | `5411`  | HTTP port used by localnet services and tools.                                                                                |
| `db-path`           | `string`   | -       | SQLite database path for persistent node state. Relative paths resolve from project root; incompatible with `--load-state`. |
...
| `block-interval-ms` | `integer`  | `500`   | Block production interval in milliseconds when automatic mining is enabled.                                                 |
| `no-mining`         | `boolean`  | `false` | Disable automatic block production; mine blocks manually with `acton simulated-localnet mine` or `/acton_mine`.                       |
...
`acton simulated-localnet start --port <PORT>`, `--db-path <PATH>`, `--fork-net <NETWORK>`, `--fork-block-number <SEQNO>`, `--accounts <NAME[,NAME...]>`, `--rate-limit <RPS>`, `--response-delay-ms <MS>`, `--block-interval-ms <MS>`, and `--no-mining` override these values for a single run. If `[localnet].no-mining = true`, there is no CLI flag that turns automatic mining back on for that run.
