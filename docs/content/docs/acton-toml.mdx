...
<Callout type="tip">
  [Acton Studio](/docs/studio) watches this manifest and rebuilds project artifacts. Its workflow proceeds from local tests through a [simulator network](/docs/environments/simulator/overview) to testnet and mainnet.

  Use [generated connection settings](/docs/environments/how-to/connect) for network entries.
</Callout>
...
The nested `wrappers.tolk` and `wrappers.typescript` tables accept the same fields as their project-level counterparts. A field set for a contract overrides only that field; omitted fields continue to use the project-level value.
...
Configures shared simulator defaults and local development services.
...
The localnet fork uses the network configuration from the resolved masterchain block. When `fork-block-number` is set, it also uses that block's Unix time; otherwise, the virtual clock starts from the current system Unix time.
...
`acton simulator start --port <PORT>`, `--db-path <PATH>`, `--fork-net <NETWORK>`, `--fork-block-number <SEQNO>`, `--accounts <NAME[,NAME...]>`, `--rate-limit <RPS>`, `--response-delay-ms <MS>`, `--block-time-ms <MS>`, and `--no-mining` override these values for a single run. If `[localnet].no-mining = true`, there is no CLI flag that turns automatic mining back on for that run.
...
The test fork uses state and network configuration from the same masterchain block. When `fork-block-number` is set, it also uses that block's Unix time; otherwise, it uses the current system Unix time.
...
The `[test.mutation]` subsection configures default filters for `acton test --mutate`. Available rule IDs are documented in [Mutation Rules](/docs/testing/mutation-testing/rules).
...
Wallet V5 derives its wallet ID from the network's TON `global-id`. Custom networks default to `-3`, matching testnet and localnet; set `global-id = -239` for a custom endpoint that is an alias or proxy for mainnet. Localnet always keeps `-3`, even when `[localnet].fork-net = "mainnet"`: the fork only provides remote state and does not turn the local chain into mainnet.
...
When `explorer` is omitted for `custom:<name>` and both `api.v2` and `api.v3` are configured, the default Actonscan link carries the network name and API endpoints as shared-network query parameters. If `api.v3` is unavailable, Acton keeps the legacy fallback derived from `api.v2`: `<scheme>://<host>/explorer/tx/<hash>`.
