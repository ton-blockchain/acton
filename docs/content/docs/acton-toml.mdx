...
Configures simulator defaults and the shared localnet target. The `acton simulator` command reads
this table.

Docker-based `acton localnet` shares the startup `accounts` selection — its node, timing, storage, and API settings use its own command options.
