...
- `rules-file` adds custom query-based rules from JSON. If a custom rule uses
  ...
  Run `acton meta get-schema mutation-rules` to print the JSON schema for that
  file format.
...
For practical workflows and CI usage, see [Linting](/docs/lint).
...
For one-off line-level suppressions in source code, use `// check-disable-next-line ...` comments (see [project lint policy configuration](/docs/lint#set-project-lint-policy)).
