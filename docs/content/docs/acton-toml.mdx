...
- For Tolk wrappers, `--output` has highest priority, then `--output-dir`, then `[wrappers.tolk].output-dir`, then the wrapper command's built-in default location.
...
| `@contracts` | `contracts`  | Local contract-side modules such as `@contracts/types`   |
| `@tests`     | `tests`      | Shared test helpers and fixtures                         |
| `@wrappers`  | `wrappers`   | Generated wrappers such as `@wrappers/Counter`          |
...
such as `acton new` and `acton init`. If you remove them manually, Acton does
not silently recreate them during normal compilation; add them back explicitly
or rerun `acton init`.
