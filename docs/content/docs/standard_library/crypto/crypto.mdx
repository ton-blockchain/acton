...
as returned by [crypto.createMnemonic](#cryptocreatemnemonic).

Example:
```tolk
val words = crypto.createMnemonic();
val kp = words.toKeyPair(words);
