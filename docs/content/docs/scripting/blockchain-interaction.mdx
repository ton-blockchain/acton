...
script runs without `--broadcast`. To query **real deployed contracts** without
...
When the script runs with `--broadcast`, the selected `--net` already becomes
...
`--fork-net` and `--broadcast` can still be combined, but in broadcast mode
...
| `--broadcast`                                  | Selected broadcast network | Real wallets | Deploy and read back live state on testnet                      |
| `--broadcast --net mainnet`                    | Real mainnet state         | Real wallets | Mainnet deployment and post-deploy verification                 |
| `--broadcast --net testnet --fork-net testnet` | Real testnet state         | Real wallets | Explicit same-network fork setup, optionally with block pinning |
