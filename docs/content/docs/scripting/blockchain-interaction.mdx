...
<Callout type="warn">
  ...
  Do not use `ExternalSendResult.isAccepted()` after broadcasting an external-in
  message. The send API confirms submission, but it does not report whether the
  destination contract accepted the message. `isAccepted()` exits with an error
  in this case. Use `waitForFirstTransaction()` or `waitForTrace()` to look for
...
Use `net.runGetMethodExt()` to inspect gas usage and the VM exit code.
Call `unwrap()` to check success and decode the result:
...
`GetMethodResult<Ret>.meta` contains `gasUsed` and `exitCode` for the contract's
get-method execution. `isSuccess()` returns `true` for VM exit codes `0` and `1`.
Neither metadata access nor `isSuccess()` decodes the result as `Ret`.

On VM failure, `unwrap()` reports the getter's error and stops execution.
For a successful nullable getter, `unwrap()` can return `null`.
If `Ret` does not match the saved stack, `unwrap()` reports a decoding error.
Repeated calls to `unwrap()` decode the same saved stack.
...
`net.runGetMethod()` checks success and decodes the result before returning it.
Both functions throw if the contract is not deployed, has no code, or the emulator
cannot run.
...
before calling the remote API. Fork runs without an explicit block also persist
accounts under the resolved sequence number.

Acton stores the matching network configuration and, for explicit historical
forks, block time for 24 hours under
`build/cache/masterchain-snapshots/<network>/<seqno>.json`. Use
`--no-fork-cache` to force fresh account and library reads for a run. This option
does not disable the masterchain snapshot cache. Use `--clear-cache` to remove
the persisted entries with the rest of `build/cache`.
