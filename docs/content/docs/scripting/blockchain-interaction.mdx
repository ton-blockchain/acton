...
For wallet resolution and broadcast routing details, see [Wallets and broadcasting](/docs/scripting/wallets-and-broadcasting). For remote reads without broadcasting, see [Querying remote state](/docs/scripting/querying-remote-state).
...
`from` matches a wallet opened at script startup. With local wallet files, Acton
signs and sends through that wallet. With `--tonconnect`, the send is routed to
the connected wallet for approval.

If `from` does not match an opened wallet address, `net.send()` falls back to
local emulator execution even when `net.isBroadcasting()` is `true`. Use
`scripts.wallet(...)` for the sender address in broadcast scripts so the routing
is explicit.

`net.sendExternal()` sends a raw external-in message directly in broadcast mode.
It is rejected with `--tonconnect`; connected wallets must approve internal
wallet messages through `net.send(...)`.
...
For read-after-send flows, wait before reading contract state from the network:

```tolk
val result = counter.sendIncreaseCounter(deployer.address, 5, { value: ton("0.05") });
if (result.waitForTrace() == null) {
    return;
}

println("Counter is now {}", counter.currentCounter());
```

The wait helpers poll the selected network in `--net` mode. Without a wait, a
getter can run before the transaction is applied or indexed.
...
When `--fork-net` is enabled, Acton can fetch account state from the real blockchain for the script execution. This allows to:
...
See [Querying remote state](/docs/scripting/querying-remote-state) for `--fork-net`, custom networks, API keys, and script read helpers.
