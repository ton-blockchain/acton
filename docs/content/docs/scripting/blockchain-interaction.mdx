...
The `--net localnet` deployment above requires the `[networks.localnet]` to be configured in `Acton.toml` with the [selected simulator endpoints](/docs/environments/connect) prior.
...
Acton resolves one masterchain block for the script run. Account state and the network configuration come from that block. Use `--fork-block-number` to select an exact block and use its Unix time. If you omit it, Acton resolves the latest block once, pins it for the script run, and uses the current system Unix time.
