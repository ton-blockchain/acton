...
Test UI is a browser interface for reviewing completed `acton test` runs with traces, transaction trees, logs, and source links.
...
Logs tab shows `Executor Log` and `VM Log` grouped by transaction for the selected trace. If a transaction has no logs, it is skipped; if all are empty, UI shows `No logs for this trace`.
