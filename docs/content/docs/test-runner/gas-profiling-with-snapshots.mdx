...
<Callout type="tip" title="How to read row groups">
...
Use [`tracing.save(...)`](/docs/standard_library/emulation/tracing/#tracingsave) after `net.send(...)`:

```tolk
import "@acton/emulation/tracing"

get fun `test-transfer-flow`() {
    // ...
    val txs = net.send(sender.address, msg);
    tracing.save(txs, "transfer-wallet-to-wallet");
...
acton test --filter "test-transfer.*" --snapshot gas-baseline.json
...
Acton prints a warning and continues without baseline comparison.
