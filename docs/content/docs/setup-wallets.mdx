...
For TonCenter-backed balance lookups, including
`acton wallet list --balance` and the interactive post-airdrop wait after
`acton wallet new`, you can set `TONCENTER_API_KEY` in the environment.
`acton wallet list --balance` also accepts `--api-key`.
