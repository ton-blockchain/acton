...
fun onInternalMessage(in: InMessage) {
    ...
    match (msg) {
        ...
        ClosePoll => {
            ...
            // Return remaining balance to the owner.
            createMessage({
                bounce: false,
                value:  0,
                dest:   storage.owner,
            }).send(SEND_MODE_CARRY_ALL_REMAINING_BALANCE | SEND_MODE_DESTROY_ACCOUNT_IF_ZERO);
        }
        else => {
...
get fun `test vote rejected after close`() {
    ...
    expect(voteRes).toHaveFailedTx({
        from:     alice.address,
        to:       poll.address,
        exitCode: Errors.PollClosed as int,
    });
}
```
...
```bash
acton test --baseline-snapshot baseline.json
```

If any test uses significantly more gas than the baseline, the command exits with an error.
