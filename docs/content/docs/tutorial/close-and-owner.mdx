...
type AllowedPollMessage = Vote | ClosePoll // [!code ++]
...
<Callout type="info">
  `PollStorage` now has an `isClosed` field. Update `setupTest()` to include `isClosed: false` in
...
get fun `test non-owner cannot close poll`() {
    ...
    val res = poll.sendClosePoll(alice.address, { value: grams("0.05") });
...
get fun `test owner closes poll`() {
    ...
    val closeRes = poll.sendClosePoll(owner.address, { value: grams("0.05") });
...
<Callout type="info">
  Re-capture the baseline after intentional changes that affect gas — for example, adding a field to
  storage.
</Callout>
