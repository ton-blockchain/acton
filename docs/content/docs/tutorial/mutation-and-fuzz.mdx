...
The earlier duplicate-vote tests should already catch removal of `assert (!storage.hasVoted) throw Errors.AlreadyVoted` (rule `remove_assert`) through Poll's final vote count. If this mutation survives in your suite, check those tests. Add a direct assertion on Voter's exit code to make the failure more specific:
...
get fun `test voter exits with AlreadyVoted on duplicate`() {
    ...
    val res1 = poll.sendVote(alice.address, 0, { value: grams("0.1") });
