...
<Callout type="info">
  Re-run `acton wrapper Poll --ts --output-dir app/wrappers-ts` whenever messages or getters change.
  The file is regenerated from scratch every time — do not edit it manually.
</Callout>

To avoid repeating `--output-dir app/wrappers-ts`, add the TypeScript wrapper
output directory to `Acton.toml`:
...
After that, regenerate the wrapper with:
