...
<Callout type="warn" title="Testnet only">
  All commands on this page target testnet. TON testnet has no real value, but treat the wallet
...
```bash
cat mnemonic.txt | acton wallet import --name testnet-deployer --local --version v5r1
```
