...
<Callout type="warn" title="Testnet only">
  All commands on this page target testnet. Testnet GRAM has no real value, but treat the wallet
  mnemonic with the same care as a mainnet wallet — never commit it to git.
</Callout>
...
`--airdrop` requests test GRAM from the configured faucet. If the faucet is rate-limited, fund the wallet manually via the [Testnet Giver Bot](https://t.me/testgiver_ton_bot).
...
<Callout type="info">
  `acton wallet list --balance` queries TON Center for live balances. Set
...
fun main() {
    ...
    val res = poll.deploy(deployer.address, { value: grams("0.05") });
...
Source verification publishes the Tolk sources that reproduce the poll's compiled code hash. Use the same source revision and compiler settings as deployment.

First, prepare verification and inspect the payment details:

```acton-cli
acton verify Poll --address <POLL_ADDRESS> --dry-run
```

Acton compiles the poll and requests a verification ticket. If the code hash is already verified, Acton prints its verifier link and exits without payment.

For new code, Acton checks that the deployed testnet contract has the same code hash. A mismatch stops verification before payment. Acton then prepares the sources and displays the ticket's payment amount, recipient address, and comment.

`--dry-run` does not send a payment or upload sources. It does not confirm that the verifier can compile the source bundle.

To complete verification, pay with the testnet wallet created earlier:

```acton-cli
acton verify Poll --address <POLL_ADDRESS> --wallet testnet-deployer
```

Review the amount and recipient, then approve the payment when prompted. The ticket sets the amount. Keep enough testnet GRAM in `testnet-deployer` for this payment and network fees.

Acton waits for the recipient transaction to finalize, then uploads the sources with that transaction's hash. On success, it prints a link that identifies the verified code hash:

```acton-cli noCopy title="terminal"
✓ Contract verification completed!
View at: https://verifier.ton.org/<CODE_HASH>
```

Open the printed link to inspect the verified sources. See the [verification guide](/docs/verify) for TON Connect, compiler options, and payment retries.

## Send a vote and retrace the transaction

Create `scripts/vote.tolk`:

```tolk title="scripts/vote.tolk"
import "@acton/io"
...
fun main(pollAddress: address, option: uint8) {
    ...
    val res = poll.sendVote(voter.address, option, { value: grams("0.1") });
    println(res.waitForTrace());
