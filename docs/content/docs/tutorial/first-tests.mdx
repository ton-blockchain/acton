...
```acton-cli noCopy title="terminal"
$ acton test --ui
  Test UI started at http://localhost:12344
```

The command will automatically open the UI in your default browser. Open the transactions tab.
The first three transactions are wallet deployments, the fourth transaction is Poll deployment, and the last one is our voting logic:
...
<Callout type="tip">
  Open the Test UI again and click the failing test. In the transactions tab, select the last
  transaction. The trace shows that the Vote transaction failed with exit code 101, and the Poll
  transaction received a bounced message, which is marked with a dotted line and is currently
  ignored by Poll contract.
</Callout>

<figure className="theme-image my-6">
  <img
    src="../test-ui-duplicate-vote-trace.png"
    alt="Test UI transactions tab showing the failed duplicate vote trace and bounced message in light theme"
    className="theme-image-light rounded-lg border shadow-sm"
  />
  <img
    src="../test-ui-duplicate-vote-trace_dark.png"
    alt="Test UI transactions tab showing the failed duplicate vote trace and bounced message in dark theme"
