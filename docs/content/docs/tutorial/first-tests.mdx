...
fun setupTest(): (Poll, Treasury, Treasury, Treasury) {
    ...
    val res = poll.deploy(owner.address, { value: grams("0.5") });
...
get fun `test alice votes option 0`() {
    ...
    val res = poll.sendVote(alice.address, 0, { value: grams("0.1") });
...
get fun `test alice and bob vote for different options`() {
    ...
    val res0 = poll.sendVote(alice.address, 0, { value: grams("0.1") });
    expect(res0).toHaveAllSuccessfulTxs();

    val res1 = poll.sendVote(bob.address, 1, { value: grams("0.1") });
...
get fun `test alice cannot vote twice`() {
    ...
    val res2 = poll.sendVote(alice.address, 0, { value: grams("0.1") });
