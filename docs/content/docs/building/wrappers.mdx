...
<Callout type="tip">
  Reuse generated wrappers through the [Studio workflow](/docs/studio): local emulator tests, scripts connected to a [simulator network](/docs/environments/connect), then testnet and mainnet. The `--net` option selects the destination network.
</Callout>
...
In local emulation, `ExternalSendResult.isAccepted()` reports whether the contract accepted the external-in message. After a network broadcast, use `waitForFirstTransaction()` or `waitForTrace()` instead: successful submission does not prove contract acceptance, so `isAccepted()` exits with an error.
