...
Use an existing `tolk-abi-to-go` executable on PATH, or pass its path explicitly:

```bash
acton wrapper Counter --go
acton wrapper --all --go --output-dir internal/contracts --go-package contracts
acton wrapper --catalog data-abis.json --go --output-dir generated --go-generator ./bin/tolk-abi-to-go
```
...
Acton never downloads the Go generator. Generated files use the public,
CGO-free `github.com/toncenter/ton-indexer/ton-index-go/index/acton` runtime.
See [Go generation](/docs/commands/wrapper#go-generation) for pinned installation,
local-build instructions, and unsupported-codec diagnostics.
