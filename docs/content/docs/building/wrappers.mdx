...
The generator's Go sources are bundled in the Acton binary and run from a temporary
module with `go run -mod=readonly -trimpath`, `CGO_ENABLED=0`, and `GOWORK=off`.
Caller `GOFLAGS` and saved build-flag defaults are ignored. Your `go.mod` and
workspace are not modified. Standard Go caches are reused; first use may download
pinned dependencies such as `tonutils-go`.

Generated files use the public, CGO-free
`github.com/ton-blockchain/acton/packages/abi-go` runtime (package `acton`).
