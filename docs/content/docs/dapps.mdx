...
Acton can generate TypeScript wrappers from Tolk contract ABIs by running [`acton wrapper --ts`](/docs/commands/wrapper). Those wrappers handle address derivation, message encoding, and getter calls, so frontend or other TypeScript code never needs hand-written serialization.
...
This produces the [app layout](/docs/projects#dapp-layout): Tolk sources under `contracts/src/`, generated TypeScript wrappers under `wrappers-ts/`, and the React frontend under `app/`.
...
Set a default output directory in `Acton.toml` to omit the flag on every invocation:
...
<Callout>
    Set `TONCENTER_MAINNET_API_KEY` and `TONCENTER_TESTNET_API_KEY` in `.env` to avoid `429` rate-limit errors. Vite inlines variables into the browser bundle, so treat these keys as public.
...
All message payloads and getter calls go through `wrappers-ts/Counter.gen.ts`, not hand-written serialization. Regenerating the wrapper by running `acton wrapper --ts` after an ABI change keeps the frontend in sync.
