...
- When developing the UI separately, the Vite dev server proxies `/api`, `/admin`, and `/emulate` to `VITE_LITENODE_PROXY_TARGET` so the frontend can talk to the same local node instance.
