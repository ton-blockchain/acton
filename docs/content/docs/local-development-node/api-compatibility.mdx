...
## Legacy v2 Compatibility Aliases

Acton keeps a few TonCenter-style compatibility aliases for older clients:

- `POST /api/v2` is the primary JSON-RPC entrypoint.
- `POST /api/v2/jsonRPC` is accepted as a legacy JSON-RPC alias.
- `POST /api/v2/v2/jsonRPC` is also accepted for clients that accidentally double-prefix the `v2` segment.
- `GET /api/v2/getShards` and `GET /api/v2/shards` resolve to the same shard-list handler.
...
<Callout type="info">
  ...
  `--rate-limit` or `[litenode].rate-limit` is enabled.
</Callout>
