...
use crate::{
    observability::{
        BlockObservation, ChainHead, ChainObservation, ElectionObservation, ElectionStage,
        ExchangeRequest, ExchangeResponse, NetworkView, NodeObservation, ObservationPayload,
        ObservationStore, ObserverIdentity, ProductionView, ShardHead, SignedObservation,
        network_id,
    },
...
/// Collection and gossip are returned to the caller so node shutdown owns every
/// background task and no observer outlives its state-directory lock.
...
async fn publication_loop(
    ...
    loop {
        ...
        if publish {
            ...
            if let Err(error) =
                publish_runtime_observation(&layout, &publication, &snapshot, &store).await
            {
            ...
            _ = interval.tick() => {
                ...
                if snapshot.observation.is_some() {
                    match collect_node_head(&layout).await {
                        Ok(head) => snapshot.node_head = head,
...
async fn publish_runtime_observation(
    ...
    let validator_keys = node_runtime
        .validator_public_keys
        .iter()
        .chain(node_runtime.validator_public_key.iter())
        .map(|key| key.to_hex())
        .collect::<BTreeSet<_>>();
    ...
    let head_seqno = chain.node_head.or(node_runtime.head_seqno);
    let network_head_seqno = chain_head
        .map(|head| head.seqno)
        .or(node_runtime.network_head_seqno)
    ...
    let node = NodeObservation {
        ...
        sync_progressed_at: node_runtime.sync_progressed_at,
        ...
        current_validator,
        next_validator,
        validator_public_key: node_runtime.validator_public_key.map(|key| key.to_base64()),
        validator_public_keys: node_runtime
            .validator_public_keys
            .into_iter()
            .map(|key| key.to_base64())
            .collect(),
        validator_adnl: node_runtime.validator_adnl.map(|key| key.to_base64()),
    };
...
impl ChainCollector {
    ...
    /// Advances the verified chain window without replacing the last complete snapshot on error.
...
/// Decodes one liteserver response and keeps only fields used by the public view.
...
/// Exchanges bounded version-vector deltas with a rotating subset of live peers.
...
/// Completes one symmetric exchange and applies the response only after validation.
...
mod tests {
    ...
    async fn heartbeat_keeps_publishing_without_chain_updates() {
        ...
        "#]]
        .assert_debug_eq(&(
            observation.payload.node.head_seqno,
            observation.payload.node.network_head_seqno,
            observation.payload.node.sync_lag_blocks,
            observation.payload.node.sync_initial_masterchain_block_time,
            observation.payload.node.sync_masterchain_block_time,
            observation.payload.node.sync_target_time,
            observation.payload.node.sync_progressed_at,
        ));
