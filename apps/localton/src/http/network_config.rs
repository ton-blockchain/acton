...
#[utoipa::path(post, path = "/v1/network/config", tag = "administration",
    request_body = UpdateRequest,
    responses((status = 200, description = "Parameter confirmed in masterchain", body = UpdateResult),
        (status = 400, description = "Invalid, stale or unconfirmed update", body = super::error::ErrorResponse)))]
