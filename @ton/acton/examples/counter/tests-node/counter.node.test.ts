...
type Treasury = ReturnType<ActonTestContext["localnet"]["treasury"]>
...
test("deploy exposes initial owner", async ({localnet}) => {
...
test("increase counter", async ({localnet}) => {
  const {contract, deployer} = await setupTest({localnet})
...
test("reset counter", async ({localnet}) => {
  const {contract, deployer} = await setupTest({localnet})
...
test("decrease counter", async ({localnet}) => {
  const {contract, deployer} = await setupTest({localnet})
...
test("decrease counter fails on underflow", async ({localnet}) => {
  const {contract, deployer} = await setupTest({localnet})
...
test("decrease counter fails on overflow", async ({localnet}) => {
  const {contract, deployer} = await setupTest({localnet})
...
test("non-owner cannot change counter", async ({localnet}) => {
  const {contract, notDeployer} = await setupTest({localnet})
...
test("unknown message", async ({localnet}) => {
  const {contract, deployer} = await setupTest({localnet})

  const res = await sendAny(
    {localnet},
    contract,
    deployer,
    beginCell().storeInt(0x00_00_09_99, 32).endCell(),
  )
  expectFailedTx(res, {
    from: deployer.address,
    to: contract.address,
  ...
  const res2 = await sendAny({localnet}, contract, deployer, Cell.EMPTY)
  expectSuccessfulTx(res2, {
    from: deployer.address,
    to: contract.address,
  })
...
async function setupTest({localnet}: ActonTestContext): Promise<TestContext> {
